...
class AddTaskUseCase(private val repository: JourneyRepository) {
    suspend operator fun invoke(journeyId: String, task: com.mymultiverse.kmp.domain.model.JourneyTask) = 
        repository.addTask(journeyId, task)
}

class UpdateTaskUseCase(private val repository: JourneyRepository) {
    suspend operator fun invoke(task: com.mymultiverse.kmp.domain.model.JourneyTask) = 
