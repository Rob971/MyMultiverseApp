...
- **Supabase is unconfigured by default.** `local.properties` has only `sdk.dir` (no `supabase.anonKey`), so the build embeds an empty key and the app launches to the login/onboarding screen with auth shown as "not available on this build" (`AuthState.ConfigurationMissing`). The UI auth gate (`presentation/App.kt`) means the household/nutrition screens are **not reachable** without a valid `supabase.anonKey` in `local.properties` — there is no UI path to local-only nutrition. To exercise authenticated grocery/meal-plan flows end to end, add `supabase.anonKey` (and apply migrations to that project) before building.
