...
Instrumented UI tests: run on **push** to `main`/`master`, **pull request**, or **workflow_dispatch** → `all` / `android-instrumented-tests`. **iOS compile** is **manual dispatch only** (macOS bills at 10×). Feature pushes run Android CI + Firebase only (Linux).
