-- P1: rename sharing-space tables to household terminology (stable UUIDs).

alter table public.sharing_spaces rename to households;
alter table public.space_members rename to household_members;
alter table public.space_invites rename to household_invites;
alter table public.space_nutrition_features rename to household_modules;
alter table public.nutrition_space_week_data rename to nutrition_household_week_data;

alter table public.household_members rename column space_id to household_id;
alter table public.household_invites rename column space_id to household_id;
alter table public.household_modules rename column space_id to household_id;
alter table public.nutrition_household_week_data rename column space_id to household_id;

alter table public.nutrition_household_week_data replica identity full;
...
begin
    if exists (
        ...
        alter publication supabase_realtime drop table public.nutrition_space_week_data;
    end if;
    if not exists (
        select 1 from pg_publication_tables
        where pubname = 'supabase_realtime'
          and schemaname = 'public'
          and tablename = 'nutrition_household_week_data'
    ) then
        alter publication supabase_realtime add table public.nutrition_household_week_data;
    end if;
end $$;
...
begin
    ...
    where household_id = v_row.space_id;
...
begin
    ...
    into v_member_count
    from public.household_members sm
    where sm.household_id = v_space_id
      and sm.left_at is null
      and sm.user_id is not null
      and sm.user_id <> v_user_id;
