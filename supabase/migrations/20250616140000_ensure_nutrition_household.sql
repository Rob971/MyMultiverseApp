-- Single-household bootstrap for Nutrition, Adventures, and Budget (one household, shared members).
-- Resolves or creates the nutrition sharing space used as the household persistence scope.
...
create or replace function public.ensure_nutrition_household()
returns json
language sql
security definer
set search_path = public
as $$
    select public.ensure_household();
$$;
...
revoke all on function public.ensure_nutrition_household() from public;
grant execute on function public.ensure_nutrition_household() to authenticated;
